  ...
  const del_schedule_mutation = client_q.crossword.schedules.delete_puzzle_schedule.useMutation({
    onSuccess() {
      startTransition(async () => {
        ...
        await invalidatePage('/padajala/schedules');
      });
    },
    onError() {
          ...
          <Card key={schedule.id} className="transition-shadow hover:shadow-md">
            <CardHeader className="items-center gap-2">
              ...
              <CardAction className="flex items-center gap-1 self-center">
                <Link
                  href={`/padajala/schedules/edit/${schedule.id}`}
...
export const PastSchedules = () => {
  ...
  const past_schedules_q = client_q.crossword.schedules.get_past_schedules.useQuery(undefined, {
