...
import type {
  ...
  CrosswordNextScheduleType
} from '~/util/cache.server/crossword_cache';
...
type Props = {
  next_schedule: CrosswordNextScheduleType;
  listed_puzzles: CrosswordListedPuzzlesType;
};

export function NoScheduledCrossword({ next_schedule, listed_puzzles }: Props) {
  ...
  useEffect(() => {
    ...
    const runRefresh = async () => {
      ...
      for (let attempt = 0; attempt <= SCHEDULE_REFRESH_MAX_RETRIES; attempt++) {
        try {
          const result = await client.crossword.refresh_current_schedule.mutate();
  ...
  return (
    <div className="w-full bg-linear-to-br from-slate-50 via-blue-50 to-indigo-50 pb-12 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900">
            ...
            >
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                  A fresh crossword is almost ready!
                </p>
              </div>
            </motion.div>
          ) : (
      ...
      {!loadingNewPuzzle ? <ListedCrosswordBrowseEmbed listed_puzzles={listed_puzzles} /> : null}
    </div>
  );
}
