...
const ignoreFailure = <A, E, R>(effect: Effect.Effect<A, E, R>) =>
  ignoreReportedFailure(effect, 'qstash.crossword.side_effect');

export const Route = createFileRoute('/api/qstash/crossword/schedule_listing')({
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        return runQstashEffect(
          Effect.gen(function* () {
            ...
            const analysis = analyzeWordPlacements(
              schedule.puzzle.grid_data,
              schedule.puzzle.word_list
            );
            if (!analysis.canList) {
              console.error(
