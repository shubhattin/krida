...
const loader$ = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data }) => {
    ...
    const schedule = await runLoaderEffect(
      dbRunHttp('crossword.admin.get_schedule_for_edit', (client) =>
...
export const Route = createFileRoute('/padajala/(auth)/_auth/schedules/edit/$id')({
  loader: async ({ params }) => {
    const { schedule } = await loader$({ data: { rawId: params.id } });
    if (!schedule) throw redirect({ to: '/padajala/schedules' });
    return { schedule };
  },
  head: () => routeHeadFromPageMeta({ title: 'Edit Crossword Schedule', project: 'padajala' }),
