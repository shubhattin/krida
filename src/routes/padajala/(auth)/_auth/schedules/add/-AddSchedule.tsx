...
function useCrosswordSearchPager(search_title: string) {
...
const fetchCrosswordListPage = async (page: number, search_title: string) =>
...
function useCrosswordListQuery(page: number, search_title: string, enabled: boolean) {
  const puzzle_list_q = useQuery({
    queryKey: ['crossword_list_schedule', page, search_title],
    queryFn: () => fetchCrosswordListPage(page, search_title),
    enabled,
...
const AddSchedule = (props: Props) => {
  ...
  const { debouncedSearchTitle, page, setPage, selectedPuzzle, setSelectedPuzzle } =
    useCrosswordSearchPager(search_title);
  ...
  const { puzzle_list, pageCount, hasPrev, hasNext, isInitialLoading, isFetching, total } =
    useCrosswordListQuery(page, debouncedSearchTitle, type === 'add');
