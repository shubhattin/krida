...
import { dbDataFileName, dbUrl, queryClient } from './client';
...
async function backup_data() {
  ...
  console.log(`Backing up ${dbMode} database...`);
  console.log('Backing up schema...');
  backup(
    `pg_dump --dbname='${dbUrl}' --if-exists --schema-only --clean --no-owner -f b.sql`,
    'db_dump_schema.sql',
    'b.sql'
  );
  console.log('Backing up data...');
  backup(
    `pg_dump --dbname='${dbUrl}' --data-only --insert --no-owner --rows-per-insert=8000 -f b.sql`,
    'db_dump_data.sql',
    'b.sql'
  );
  ...
  await import_data(false).then(() => {
    queryClient.end();
    fs.copyFileSync(`./out/${dbDataFileName}`, './backup/db_data.json');
  });
